...
class Ui_Form(object):
    ...
    def retranslateUi(self, Form):
        ...
        self.testAll.setToolTip(QCoreApplication.translate("Form", u"Warning, this can take some time!", None))
...
#if QT_CONFIG(tooltip)
        self.labelCommonPsets.setToolTip(QCoreApplication.translate("Form", u"<html><head/><body><p>The IFC standard offers standard, predefined property sets for many object types. for example, the property set Pset_WallCommon contains properties that the IFC standard thinks all walls should have. This test will check that all BIM objects have the right property set, if available.</p><p>Note that this is by no means a formal requirement, and these will inflate the size of your IFC file consequently. We suggest you add standard property sets only if you are actually using any of them.</p></body></html>", None))
#endif // QT_CONFIG(tooltip)
